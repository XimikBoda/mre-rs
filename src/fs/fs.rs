...
pub fn remove_file(path: &Path) -> Result<(), i32> {
    ...
    let res = unsafe{ vm_file_delete(ucs2_path.as_ptr()) };
...
pub fn rename(from: &Path, to: &Path) -> Result<(), i32> {
    ...
    let res = unsafe{ vm_file_rename(ucs2_from.as_ptr(), ucs2_to.as_ptr()) };
...
pub fn create_dir(path: &Path) -> Result<(), i32> {
    let ucs2_path = path.as_mre_str();
    let res = unsafe{ vm_file_mkdir(ucs2_path.as_ptr()) };
...
pub fn remove_dir(path: &Path) -> Result<(), i32> {
    let ucs2_path = path.as_mre_str();
    let res = unsafe{ vm_file_rmdir(ucs2_path.as_ptr()) };
